...
void drawLines(Mat &src, vector<Vec4i> &lines, int offsetX, int offsetY) {
    for( size_t i = 0; i < lines.size(); i++ ) {
        Vec4i l = lines[i];
        line(src, Point(l[0] + offsetX, l[1] + offsetY), Point(l[2] + offsetX, l[3] + offsetY), Scalar(255), 2, LINE_AA);
    }
}

void drawPoint(Mat &src, vector<Point> &points, Scalar color) {
    for( size_t i = 0; i < points.size(); i++ ) {
        Point p = points[i];
        circle(src, p, 8, color, FILLED);
        LOGD("point: (%i, %i)", p.x, p.y);
    }
}

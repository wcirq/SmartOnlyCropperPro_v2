...
public class CropImageView extends ImageView {
    ...
    Point[] mSrcCropPoints; // 原始的裁剪区域（未拖动调整的）, 0->LeftTop, 1->RightTop， 2->RightBottom, 3->LeftBottom
    Point[] mCropPoints; // 裁剪区域, 0->LeftTop, 1->RightTop， 2->RightBottom, 3->LeftBottom
    ArrayList<Point> mCropEdges; // 裁剪区域, 0->LeftTop, 1->RightTop， 2->RightBottom, 3->LeftBottom
    ...
    public void setCropPoints(Point[] cropPoints) {
        ...
        if (!checkPoints(cropPoints)) {
            ...
            this.mSrcCropPoints = new Point[cropPoints.length];
            for (int i=0;i<cropPoints.length;i++){
                this.mSrcCropPoints[i] = new Point(cropPoints[i].x, cropPoints[i].y);
            }

            this.mCropPoints = cropPoints.clone();
            invalidate();
        }
    }
    ...
    /**
     * 设置选区详细边缘
     * @param cropEdges 选区详细边缘
     */
    public void setCropEdges(ArrayList<Point> cropEdges) {
        if (getDrawable() == null) {
            Log.w(TAG, "should call after set drawable");
            return;
        }
    ...
    public void setImageToCrop(Bitmap bmp) {
        ...
        if (mAutoScanEnable){
            SmartCropper.RectangularInfo rectangularInfo = SmartCropper.scan(bmp, null);
            setCropPoints(rectangularInfo.getVertex());
            setCropEdges(rectangularInfo.getEdges());
        }else  {
            setCropPoints(null);
            setCropEdges(null);
        }

        Log.d(TAG, "scan cost time:" +( System.currentTimeMillis() - startTime));
    }
     ...
     */
    public Bitmap crop() {
        return crop(mSrcCropPoints, mCropPoints, mCropEdges);
    }
    ...
    public Bitmap crop(Point[] points) {
        ...
        return bmp == null ? null : SmartCropper.crop(bmp, points);  // 原始的4点裁剪
    }
     ...
     * @return 裁剪后的图片
     */
    public Bitmap crop(Point[] srcPints, Point[] points, ArrayList<Point> edges) {
        if (!checkPoints(points)) {
            return null;
        }
        Bitmap bmp = getBitmap();
        return bmp == null ? null : SmartCropper.crop(bmp, srcPints, points, edges);  // 优化后的拉伸裁剪
    }
